...
public class Merge {
  ...
  public static Frame merge(final Frame leftFrame, final Frame riteFrame, final int leftCols[], final int riteCols[], boolean allLeft, int[][] id_maps) {
    ...
    final BigInteger leftBaseD = leftIndex._isNotDouble[0] ? BigInteger.ZERO : leftIndex._baseD[0];
    final BigInteger riteBaseD = hasRite ? (riteIndex._isNotDouble[0] ? BigInteger.ZERO : riteIndex._baseD[0]) : leftBaseD;

    long leftMSBfrom = leftIndex._isNotDouble[0]?(riteBase - leftBase) >> leftShift:riteBaseD.subtract(leftBaseD).shiftRight(leftShift).longValue();
    boolean riteBaseExceedsleftBase=leftIndex._isNotDouble[0]?(riteBase>leftBase):riteBaseD.compareTo(leftBaseD)>0;
