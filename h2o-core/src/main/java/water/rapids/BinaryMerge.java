...
class BinaryMerge extends DTask<BinaryMerge> {
  ...
  static class FFSB extends Iced<FFSB> {
    ...
    FFSB( Frame frame, int msb, int shift, int fieldSizes[], long base[], BigInteger baseD[], boolean isNotDouble[]) {
 ...
 //   long max() { return (((long)_msb+1) << _shift) + _base[0]-2; } // the last  key possible in this bucket
    ...
    long min() {
      return _isNotDouble[0]?(((long)_msb) << _shift) + _base[0]-1:BigInteger.valueOf(((long)_msb) << _shift).add(_baseD[0].subtract(BigInteger.ONE)).longValue();
    }
    long max() {
      return _isNotDouble[0]?(((long)_msb+1) << _shift) + _base[0]-1:BigInteger.valueOf(((long)_msb+1) << _shift).add(_baseD[0].subtract(BigInteger.ONE).subtract(BigInteger.ONE)).longValue();
    }
  }
